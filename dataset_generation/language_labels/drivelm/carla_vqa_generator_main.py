...
def parse_arguments():
    ...
    path_group.add_argument('--output-directory', type=str, default='database/simlingo_v2_2025_01_10/drivelmTD',
                            help='Output directory for the vqa-graph')
    path_group.add_argument('--output-graph-examples-directory', type=str, default='database/simlingo_v2_2025_01_10/drivelmTD',
...
def convert_carla_to_nuscenes_and_save(args, carla_file_content):
    ...
    for path, tick in zip(carla_file_content['image_paths'], carla_file_content['llava_format']):
        ...
        # easier to debug:
        save_dir = path.replace('/data/', '/drivelm/').replace('.jpg', '.json').replace('rgb', 'vqa')
        # final version
